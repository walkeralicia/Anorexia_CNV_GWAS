...
#======================= This script prepares the developmental CNV list ============================================================================
...
data <- "data" ## path to data folder with CNV_List.xlsx and NRXN1_exons.tsv
cnv_path <- "/Anorexia/UKB/burden_analysis/devCNVs" ## path to conduct developmental-CNV burden analyses

#==== Format CNV list into .bed format and separate by CNV type (duplications and deletions) =======
cnvs <- read_excel(paste(data, "CNV_List.xlsx", sep="/"), sheet=1)
...
df <- fread(paste(cnv_path, "NRXN1_exons.tsv", sep="/"),fill=TRUE, sep=",") %>% as.data.frame()
exons <- data.frame()
for (i in 1:nrow(df)){
  chr=2
