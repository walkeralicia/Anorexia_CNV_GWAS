...
ukb <- read.table("/QRISdata/Q4399/Anorexia/UKB/burden_analysis/devCNVs/devCNVs_unweighted.txt", header=T)
angi <- read.table("/QRISdata/Q4399/Anorexia/ANGI/results/angi_devCNVs_unweighted.txt", header=T)
...
cnvs <- read_excel("/QRISdata/Q4399/Anorexia/CNV_List.xlsx", sheet=1)
...
res2 <- comb %>% 
  select(ID, CNV, Location, Genotype, Size, pHaplo, pTtriplo, Rees, Marshall,
         ...
         Zvalue.y, Pvalue.y, Stouffer_Zvalue,Stouffer_Pvalue, OR_meta, CI_meta)


colnames(res2) <- c("ID", "CNV", "Location","Genotype","Size",
                    "pHaplo", "pTriplo", "Rees", "Marshall",
                    "ANGI_Case_N", "ANGI_Cont_N", "ANGI_Cont_SE", 
                    "ANGI_Cont_Freq_10K", "ANGI_Cont_CI", "ANGI_OR", 
                    "ANGI_CI", "ANGI_Zvalue", "ANGI_Pvalue",
                    "UKB_Case_N", "UKB_Cont_N", "UKB_Cont_SE", "UKB_Cont_Freq_10K", 
                    "UKB_Cont_CI", "UKB_OR", "UKB_CI", "UKB_Zvalue", "UKB_Pvalue",
                    "Stouffer_Zvalue","Stouffer_Pvalue", "Meta_OR", "Meta_CI")
...
final <- RES[order(RES$Stouffer_Pvalue),]
...
final2 <- final[, c(-1)]
write.csv(final2, "/QRISdata/Q4399/Anorexia/UKB/burden_analysis/devCNVs/devCNVs_all.csv",  row.names=F)

paper <- final2 %>% filter(Stouffer_Pvalue<0.05)

write.csv(paper, "/QRISdata/Q4399/Anorexia/UKB/burden_analysis/devCNVs/devCNVs_sig.csv",  row.names=F)




## Stouffer meta-analyse BMI results (AN controls)
#module load R/3.6
...
## a lot of cnvs that had a negbeta in ANGI had a posbeta in UKB (when looking across all)
ukb <- read.table("/QRISdata/Q4399/Anorexia/UKB/burden_analysis/BMI/devCNVs_BMI_controls_UKB.txt", header=T)
angi <- read.table("/QRISdata/Q4399/Anorexia/ANGI/results/devCNVs_BMI_controls_ANGI.txt", header=T)
...
colnames(res2) <- c("ID", "CNV", "Location","Genotype","Size","ANGI_Beta", 
                    "ANGI_CI", "ANGI_Zvalue", "ANGI_Pvalue",
                    "ANGI_N_CNVs", "ANGI_Avg_wCNV", "ANGI_Avg_woCNV",
                    "UKB_Beta", "UKB_CI", "UKB_Zvalue", "UKB_Pvalue",
                    "UKB_N_CNVs", "UKB_Avg_wCNV", "UKB_Avg_woCNV",
                    "Stouffer_Zvalue","Stouffer_Pvalue", "Beta_OR", "Meta_CI")
...
final$Stouffer_Zvalue <- signif(final$Stouffer_Zvalue, 3)
final$Stouffer_Pvalue <- signif(final$Stouffer_Pvalue, 3)
final$Bonf_Stouffer_Pvalue <- final$Stouffer_Pvalue*67
final <- final[order(final$Stouffer_Pvalue),]

final2 <- final[, c(-1)]
write.csv(final2, "/QRISdata/Q4399/Anorexia/UKB/burden_analysis/BMI/devCNVs_BMI_controls.csv",  row.names=F)

paper <- final2 %>% filter(Stouffer_Pvalue<0.05)
write.csv(paper, "/QRISdata/Q4399/Anorexia/UKB/burden_analysis/BMI/devCNVs_BMI_controls_sig.csv",  row.names=F)

## Stouffer meta-analyse BMI results (AN cases Individuals)
module load R/3.6
export R_LIBS=/scratch/90days/uqawal15/R_libraries/rlib_3.6
R
library(readxl)
...
## Read in UKB and ANGI results 
ukb <- read.table("/QRISdata/Q4399/Anorexia/UKB/burden_analysis/BMI/devCNVs_BMI_cases_UKB.txt", header=T)
angi <- read.table("/QRISdata/Q4399/Anorexia/ANGI/results/devCNVs_BMI_cases_ANGI.txt", header=T)
stouffer <- merge(angi, ukb, by = c("Index"), all=TRUE)
...
stouffer$se_meta <- 1/sqrt(1/(stouffer$se.x*stouffer$se.x) +1/(stouffer$se.y*stouffer$se.y))
stouffer$dp <- nchar(stouffer$OR_meta)-2
stouffer$LCI_meta <- round((stouffer$OR_meta-1.96*stouffer$se_meta), stouffer$dp)
stouffer$HCI_meta <- round((stouffer$OR_meta+1.96*stouffer$se_meta), stouffer$dp)
stouffer$CI_meta <- paste0("(", stouffer$LCI_meta, "-", stouffer$HCI_meta, ")")
...
final2 <- final[, c(-1)]
write.csv(final2, "/QRISdata/Q4399/Anorexia/UKB/burden_analysis/BMI/devCNVs_BMI_cases.csv",  row.names=F)

paper <- final2 %>% filter(Stouffer_Pvalue<0.05)
write.csv(paper, "/QRISdata/Q4399/Anorexia/UKB/burden_analysis/BMI/devCNVs_BMI_cases_sig.csv",  row.names=F)
